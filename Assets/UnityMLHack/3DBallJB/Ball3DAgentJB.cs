...
public class Ball3DAgentJB : Agent
{
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        if (brain.brainParameters.vectorActionSpaceType == SpaceType.continuous)
        {
            ...
            float vely = ball.GetComponent<Rigidbody>().velocity.magnitude;
            float velabs = Mathf.Abs(vely);
            float velmax = Mathf.Min(velabs, 10f);

            // Reward for velocity
            float rew = (10f - velmax) / 10;
            // Debug.Log($"Velocity Reward: {rew}");
            SetReward(rew);
